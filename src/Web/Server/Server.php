...
class Server
{
    ...
    public function request($request, $response)
    {
        ...
        $this->monitor(
            $begin, microtime(true), $request->server['request_uri'] ?? '/', $this->getData($request), $request->server['remote_addr'] ?? '', $time, $httpCode,
            $body, $traceId, $result['class'] ?? '', $result['method'] ?? '', $request->server['request_method']
        );
    }
     ...
     * @return string
     */
    private function getData(\Swoole\Http\Request $request)
    {
    ...
    private function monitor($begin, $end, $uri, $params, $ip, $time, $code, $body, $traceId, $class, $method, $reqMethod)
    {
