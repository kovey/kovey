...
class Container implements ContainerInterface
{
    ...
    public function get(string $class, string $traceId, ...$args)
    {
        ...
        if (isset($this->instances[$class->getName()])) {
            return $this->bind($class, $this->instances[$class->getName()], $traceId, $args);
        }
     ...
     * @return mixed
     */
    private function bind(\ReflectionClass $class, Array $dependencies, $traceId, Array $args = array())
    {
        $obj = null;
        if (count($args) > 0) {
        ...
        foreach ($dependencies as $dependency) {
            $dep = $this->bind($dependency['class'], $this->instances[$dependency['class']->getName()] ?? array(), $traceId);
